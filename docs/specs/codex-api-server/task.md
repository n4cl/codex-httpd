...
## 先に決めること（要確認一覧）
- [ ] A3. rollback 失敗時のエラー契約を確定する（対応: 8.2）
  - SSE/HTTP の通知内容、ログ粒度、運用時の判定基準
- [ ] A4. 同一 thread の競合ポリシーを確定する（対応: 9.1）
  - 自動キャンセルか `409` かを最終決定する
...
- [ ] 7.1 `GET /threads/{threadId}/turns/{turnId}/events` を実装する
  ...
  - 終端イベント（`final` / `cancelled` / `error`）は1回のみ配信する
  - 各イベントに `eventId`（SSE `id`）を付与する
  - 配信保証は `at-least-once`（重複配信あり）として扱う
