...
- [ ] 3.3 認証導入の拡張点を確保する（保留: 現時点では実装対象外）
  - 本項目は将来導入のための設計トレースとして管理し、現時点ではコード実装を行わない
  - 認証方式（トークン種別・失効・配布）は後続タスクで決定し、方針確定後に再オープンする
  - 設計参照: 要件 > 認証拡張余地 / 非要件
  - 完了の定義: `design.md` に要件が維持され、`task.md` に見送り理由と再開条件が明記されている

- [ ] 4. Codex app-server 接続機能を実装する
- [x] 4.1 app-server 子プロセス管理を実装する
