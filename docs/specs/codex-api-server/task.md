...
- [ ] 6.4 複雑化時のユースケース分割方針を実装へ反映する [要確認]
  - 手順が肥大化したユースケースを分割し、責務を局所化する
  - 分割判断基準（分岐増加・再利用・テスト困難）を実装レビュー観点として運用する
  - 設計参照: アーキテクチャ方針 > ユースケース分割方針
  - 完了の定義: 主要ユースケースで分割判断を説明でき、責務境界が維持される
